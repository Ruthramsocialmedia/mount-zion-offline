/* Mount Zion CBSE Tour - Service Worker */
const CACHE_NAME = 'mountzion-cbse-offline-v1';
      ...
      return Promise.allSettled(
        CORE_FILES.map((url) =>
          cache.add(new Request(url, { cache: 'reload' })).catch((err) => {
            console.warn('[SW-CBSE] Skip precache item:', url, err.message);
          })
        )
      );
  ...
  event.respondWith(
    caches.match(event.request).then((cachedResponse) => {
      ...
      return fetch(event.request).then((networkResponse) => {
        ...
        console.warn('[SW-CBSE] Offline fetch fallback:', event.request.url);
      });
    })
  );
});
